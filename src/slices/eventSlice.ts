...
export const postNewEvent = createAppAsyncThunk('events/postNewEvent', async (params: {
	...
	if (values.sourceMode === "UPLOAD") {
		...
		if (sourceMetadata.UPLOAD) {
			for (const sourceUploadMetadata of sourceMetadata.UPLOAD.metadata) {
				metadataFields = metadataFields.concat({
					id: sourceUploadMetadata.id,
					value: values[sourceUploadMetadata.id],
					type: sourceUploadMetadata.type,
					tabindex: sourceUploadMetadata.tabindex,
				});
			}
		}
	}
...
// @ts-expect-error TS(2322): Type '{ rrule: string; start: Date; device: any; i... Remove this comment to see the full error message
	...
	for (const uploadAssetOption of uploadAssetOptions) {
		if (
			uploadAssetOption.type === "track" &&
			values.sourceMode === "UPLOAD"
		) {
			let asset = values.uploadAssetsTrack?.find(
				(asset) => asset.id === uploadAssetOption.id
			);
			if (!!asset && !!asset.file && asset.file[0]) {
				if (asset.multiple) {
			...
			assets.options = assets.options.concat(uploadAssetOption);
		} else {
			if (
				!!values[uploadAssetOption.id] &&
				values.sourceMode === "UPLOAD"
			) {
				formData.append(
					uploadAssetOption.id + ".0",
					String(values[uploadAssetOption.id])
				);
				assets.options = assets.options.concat(uploadAssetOption);
			}
		}
